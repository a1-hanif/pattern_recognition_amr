...
def create_dendrogram(linkage_matrix: np.ndarray,
    ...
    color_threshold : float, optional
        Threshold for coloring clusters. If not provided but n_clusters is,
        threshold will be calculated for n_clusters.
    n_clusters : int, optional
        Number of clusters to color. Used to calculate color_threshold.
    save_path : str, optional
        Path to save the figure
    ...
    matplotlib.Figure
        Dendrogram figure
    """
    # Calculate color_threshold for n_clusters if specified
    if color_threshold is None and n_clusters is not None:
        # To get k clusters, set threshold just below the (n-k+1)th merge distance
        # where n is total samples. The linkage matrix Z has shape (n-1, 4).
        # Z[-(k-1), 2] is the distance at which we go from k to k-1 clusters.
        # So threshold should be just below Z[-(n_clusters-1), 2]
        if n_clusters > 1 and len(linkage_matrix) >= n_clusters - 1:
            # Get the distance just below the merge that reduces clusters to n_clusters-1
            color_threshold = linkage_matrix[-(n_clusters - 1), 2] - 0.01
        else:
            color_threshold = None  # Fall back to default
    ...
    dendro = dendrogram(
        ...
        leaf_rotation=90,
        leaf_font_size=6,  # Smaller font for many isolates
    ...
    # Add Legend for Clusters (use CLUSTER_COLORS for k clusters)
    if n_clusters is not None:
        legend_handles = []
        for i in range(n_clusters):
            color = CLUSTER_COLORS[i % len(CLUSTER_COLORS)]
            legend_handles.append(Patch(facecolor=color, edgecolor='black', label=f'Cluster {i+1}'))
        ax.legend(handles=legend_handles, title='Clusters (k=4)', loc='upper right')
...
def generate_all_visualizations(df: pd.DataFrame,
    ...
    fig = create_dendrogram(linkage_matrix, figsize=(16, 10), n_clusters=4, save_path=dendro_path)
