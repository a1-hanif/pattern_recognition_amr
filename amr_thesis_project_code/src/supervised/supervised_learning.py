...
def plot_all_confusion_matrices(model_results: Dict, class_names: List[str], output_path: str):
    """
    ...
    """
    # Filter for the 3 main models we want to show
    target_models = ['Logistic Regression', 'Random Forest', 'k-Nearest Neighbors']
    # Use existing models if names differ slightly, or just use keys
    models_to_plot = [m for m in target_models if m in model_results]
    
    if not models_to_plot:
        models_to_plot = list(model_results.keys())[:3]
...
def plot_feature_importance(importance_data: Dict, output_path: str, top_n: int = 10):
    """
    ...
    importance_data : dict
        Importance data from feature_importance['scores']
    output_path : str
        Path to save the figure
    top_n : int
        Number of features to plot
    """
    if not importance_data or 'scores' not in importance_data:
        return
...
def run_supervised_pipeline(df: pd.DataFrame,
    ...
    # ===========================================================================
    #  PLOT COMBINED CONFUSION MATRICES (Phase 3 Requirement 4.1)
    # ===========================================================================
    print("\n4b. Generating Combined Confusion Matrix Plot...")
    ...
    cm_output_path = output_dir / "combined_confusion_matrices.png"
    
    # Get class names from first model (they should be same for all)
    first_model_res = next(iter(model_results.values()))
    class_names = first_model_res['metrics'].get('class_names', 
                  first_model_res['metrics'].get('class_labels'))
    
    plot_all_confusion_matrices(model_results, class_names, str(cm_output_path))
    print(f"    Saved combined plot to: {cm_output_path}")
    ...
    # Generate feature importance plot (Phase 4.1 Addition)
    try:
        from config import FIGURES_DIR
        output_dir = FIGURES_DIR
    except ImportError:
    ...
    fi_output_path = output_dir / "feature_importance.png"
    plot_feature_importance(feature_imp, str(fi_output_path))
    print(f"   Saved feature importance plot to: {fi_output_path}")
